...
def valuesF(self = None):
    ...
    prob_fail =     {0:(0.00072679, 0.00145361),1:(0.00066020, 0.00132043), 2:(0.00097032,0.00194065),\
                    3:(0.00179699, 0.00359402), 4:(0.00073249,0.00146503), 5:(0.00014269,0.00028542),\
                    6:(0.00061739, 0.00123480), 7:(0.00079897,0.00138793), 8:(0.00018227,0.00098241),\
                    9:(0.00015664, 0.00097632), 10:(0.00039977,0.00087241)}
    vel_download = {0:(2.15,3.26), 1:(1.21,2.41), 2:(3.07,3.32), 3:(2.01,3.20), 4:(2.17,2.36), 5:(0.72,0.76),\
                    6:(0.68,0.72), 7:(2.54,3.18), 8:(2.49,3.09),9:(2.01,2.98), 10:(2.30, 3.12)}
    vel_upload =   {0:(1.79,3.24), 1:(0.91,1.70), 2:(2.59,3.05), 3:(1.91,3.27), 4:(1.24,1.93), 5:(0.11,0.65),
                    6:(0.52,0.73), 7:(2.32,3.14), 8:(0.70,1.86), 9:(2.05,3.45), 10:(1.31,3.17)}
...
class batchGenerator(object):
    ...
    def random_number_in_range_of_len_elements (self, batch_size  = None, len_elements = None):
        ...
        if self.config.variable_length:
            if len_elements is None: raise ValueError('must provide len_elements with varible length')
            elements_to_restric_behavior = []
            for len_element in len_elements:
                elements_to_restric_behavior.append(torch.randint(2, len_element+1, size = (1,)))
            elements_to_restric_behavior = torch.cat(elements_to_restric_behavior, dim = 0)
        
        else:
            if batch_size is None:
                batch_size = self.batch_size 
            elements_to_restric_behavior = torch.randint(2,self.max_length+1 , size = (batch_size,))
    ...
    def generate_elements_list(self, batch_size = None, max_length = None, variable = False):
            ...
            for ele_len in elements_length:
                ...
                for parameter in parameters_min_max:
                    ...
                    for  i in index:
                        ...
                        parameter_val_buffer.append(np.random.uniform(low = low, high =high, size = (1,)))
                    element.append(np.concatenate(parameter_val_buffer, axis = 0)) # -> (len(parameters), ele_len)
        ...
        parameters_min_max = parameters_min_max[:self.parameters] #Parameters: pr_error, vel_download, vel_upload
        if batch_size is None:
            batch_size = self.batch_size
        ...
        if variable:
            elements_length = np.random.randint(low = 2, high = max_length+1, size =(batch_size,))
            elements_secuences, index_elements = for_secuence(elements_length, parameters_min_max)
    ...
    def generate_batch(self, batch_size = None, max_length = None):
        ...
        parameters = parameters[:self.parameters] # parameters: pr_error, vel_download, vel_upload
        ...
        for idx in range(batch_size):
            ...
            for i in indices:
                ...
                for parameter in parameters:
                    ...
                    element.append(np.random.uniform(low = minimo, high = maximo, size = ( 1,) ) )
    ...
    def general_norm(self, parameters = 1):
        ...
        batch = np.random.uniform(0, 1, size = (self.batch_size, parameters, self.max_length+1 ))

        return batch
    

    def getnames(self, indices):
        names = {0:'GoogleDrive', 1:'OneDrive', 2: 'DropBox', 3: 'Box', 4: 'Egnyte', 5: 'ShareFile', 6: 'SalesForce',\
                    7: 'Alibaba cloud', 8:' Amazon Cloud Drive', 9: 'Apple iCloud', 10: 'Azure Storage'}
