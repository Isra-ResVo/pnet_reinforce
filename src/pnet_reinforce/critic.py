...
class Critic (nn.Module):
    def __init__(self,config,device):
        ...
        self.attention_Wref = nn.Conv1d(in_channels = self.num_neurons*2, out_channels = self.num_neurons*2, kernel_size = 1, stride = 1, bias = False)
        self.attention_Wq = nn.Conv1d(in_channels = self.num_neurons*2, out_channels = self.num_neurons*2, kernel_size = 1, stride = 1, bias = False)
        self.attention_v = nn.Conv1d(in_channels = self.num_neurons*2, out_channels = 1, kernel_size = 1, stride = 1, bias = False)
        self.Linear_1 = nn.Linear(in_features = self.num_neurons*2, out_features= self.num_neurons*2, bias = True)
        self.Linear_2 = nn.Linear(in_features = self.num_neurons*2, out_features = 1 , bias = True)


        #first element linear layer
        self.linear_first_entry = nn.Linear(in_features = 2, out_features = self.num_neurons*2, bias = True)
    ...
    def forward (self,x, kwargs):
        ...
        c_state = encoder_states[1].permute(1,2,0).reshape(batch_size, self.num_neurons*2, 1)    # (1,batch, hidden_dim) to (batch, hidden_dim, 1)

        first_entry = self.first_entry(batch_size, kwargs)
        encoder_output = torch.cat((first_entry,encoder_output), dim =0)
        arrange_input = encoder_output.permute(1,2,0) # (steps, batch, hidden_dim) to (batch, hidden_dim, steps)
    ...
    def first_entry (self = None, batchSize = None, kwargs = None):
        ...
        first_element = torch.tensor([1.1,1.4], dtype = torch.float32).repeat(batchSize,1).to(self.device)
...
class Critic_emb (nn.Module):
    def __init__(self,config):
        ...
        self.embed_1D = nn.Conv1d(in_channels = self.input_dim,out_channels = self.input_embed, kernel_size=1, stride = 1)
