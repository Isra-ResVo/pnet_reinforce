...
class Actor (nn.Module):
    def __init__(self, config, device):
        ...
        self.Linear_dummies = nn.Linear(in_features = 4, out_features = self.num_neurons*2, bias = True)
    ...
    def forward (self = None, x = None, kwargs = None):
        ...
        self.log.info('Dimenciíon del batch es al cantidad de iteracines que tendra que hacer: %s', str(x.shape[2]))
        ...
        if  self.extraElements == 'afterDecoder':
            ...
            if self.config.mode == 'k': raise NotImplementedError ('not implemented for mode = k')
            dummies = self.extraPositions()
            encoder_output = torch.cat((encoder_output,dummies), dim = 0)

        # Decoder part
        selections, log_probs = self.decoder_pointer(encoder_output, encoder_states, kwargs)
    ...
    def extraPositions(self):
        dummy_k = torch.ones(size=(self.batch_qnt,4),dtype = torch.float32, device = self.device, requires_grad = False)
        dummy_end_k = torch.ones(size=(self.batch_qnt,4),dtype = torch.float32, device = self.device, requires_grad = False)* 0.5
...
class Embedding (nn.Module):
    def __init__ (self,config):
        ...
        self.embedded_Conv1D = nn.Conv1d(in_channels= self.input_dim, out_channels= self.input_embed, kernel_size = 1, stride = 1, bias = True)
        nn.init.xavier_normal_(self.embedded_Conv1D.weight)
        self.normalize = nn.BatchNorm1d(self.input_embed)

        self.lstm = nn.LSTM(input_size = self.input_embed, hidden_size = self.num_neurons, num_layers = 1, batch_first = False, bidirectional= True)
...
def error_function(self = None, batch = None, subsets: "list" = None, only_clouds: "list" = None) -> "float":
        ...
        for idx_batch,(subset, clouds) in enumerate(zip(subsets,only_clouds)):
            ...
            clouds = torch.sort(clouds)[0] # This is necesary to get the same result (float point operations)
            combinations = itertools.combinations(clouds,subset) # -> iter_object
            combinations  = torch.stack([torch.stack(comb) for comb in combinations]) # -> Tensor
            ...
            for combination in combinations:
                ...
                mask = ~torch.sum(clouds == combination.reshape(-1,1), dim = 0).type(torch.bool)
...
def pr_error_bound(
    ...
    for i, (subset, clouds, binomial) in enumerate(zip(subsets, only_clouds, logBinomialCoefficient)):
        ...
        maskComplement = torch.tensor([cloud not in combination for cloud in clouds], dtype = torch.bool)
...
class Reward (object):
    ...
    def __init__(self, selections,device, qnt_steps, config, value_k = None):
        ...
        if self.config.mode == 'k':
            if value_k is None: raise ValueError(' value_k must be given')
    ...
    def get_k_and_clouds(self, value_k = None): 
        ...
        if self.config.mode == 'k':
            # validadation
            if value_k is None: raise ValueError('value_k is None, k must be provided')
            k_sum = value_k
    ...
    def main(self, kwargs) -> torch.Tensor:
        ...
        size_subsets = (self.sel_n - self.k_sum) + 1 #  subset size to have to fail before +2
        pr_error = self.error_formula(batch = kwargs['batch'], subsets = size_subsets, only_clouds = self.only_clouds)
        ...
        reward['normRed'] = (reward[ 'redundancy'] - minimum) / (maximum - minimum + epsilon)
        # print('valores normalizados de redundancia', reward['normRed'])

        # Ponderate
        reward['ponderate'] = self.config.wo[0]*reward['normError'] + self.config.wo[1]*reward['normRed']
    ...
    def min_max_redundancy(self, len_elements):
        ...
        # return onluy one number
            ...
            minimum = torch.ones(size =(batch_size,), dtype = torch.float32, device = self.device)*2
        ...
        else: raise ValueError('not valid mode, it\'s only accepted n, k and n_k')
    ...
    def min_max_error(self, kwargs) -> torch.Tensor:
        ...
        def indices_variable_length_and_restriction(restriction, len_elements, batch, largest):
            indices = []
            for  n, len_n_instance, batch_ele in zip(restriction, len_elements, batch):
                    indices.append(torch.topk(batch_ele[0,:len_n_instance], k = n, largest = largest))
            return indices
        ...
        if self.config.mode == 'k_n':   # warning
            ...
            if self.config.variable_length and 'len_elements' in kwargs:
                for  len_n_instance in kwargs['len_elements']:
                    indices.append(torch.arange(0,len_n_instance, dtype = torch.int64, device = self.device))
            else:
                indices = torch.arange(0, qnt, dtype = torch.int64, device = self.device ).repeat(siz,1)
            ...
            if self.config.variable_length and 'len_elements' in kwargs:
                indices = indices_variable_length_and_restriction(kwargs['restricted_n'], kwargs['len_elements'], batch, True)
        ...
        else: raise ValueError("This value is not valid, only accept n, k and n_k configurations")
            
        max_error = self.error_formula(batch = batch, subsets = subsets, only_clouds = indices)
        ...
        if  self.config.mode == 'k_n':
            ...
            if self.config.variable_length:
                ...
                val = qnt
                indices = torch.arange(start = 0, end = qnt, dtype = torch.int64, device = self.device).repeat(siz,1)
                subsets = (torch.ones(size=(siz,), dtype = torch.int64, device = self.device)*val)-1

        elif self.config.mode == 'n' and 'restricted_n' in kwargs:
            if self.config.variable_length and 'len_elements' in kwargs:
                indices = indices_variable_length_and_restriction(kwargs['restricted_n'], kwargs['len_elements'], batch, False)
                subsets = [i - 1 for i in kwargs['len_elements']]
            ...
            if self.config.variable_length:
                indices = indices_variable_length_and_restriction(kwargs['len_elements'], kwargs['len_elements'], batch, False)
                subsets = [i-j+1 for i,j in zip(kwargs['len_elements'], kwargs['restricted_k'])]
            else:
                ...
                subsets = torch.ones(size=(siz,), dtype = torch.int64, device = self.device)*val
        else: raise ValueError('One condition is violated')

      
        min_error =self.error_formula(batch = batch, subsets = subsets, only_clouds = indices)
    ...
    def redundancy(self):
        ...
        for (k_i,n_i) in zip(self.k_sum , self.sel_n):
            ...
            rewardR.append(torch.tensor(self.dictRedundancy.get(key, None), dtype = torch.float32, device = self.device)) 
    ...
    def extraction_time(self):
        ...
        for (k_i, n_i) in zip(self.k_sum, self.cl):
            ...
            k = str(k_i.cpu().numpy())
            n = str(n_i.cpu().numpy())
            rwd_time.append(torch.tensor(self.time.get(k+n, 12345))) #se castigan valores  soluciones no validas
...
def pr_vals_2_plot(toCompareInPlot, kwargs, point, config, device, idxEle = 0):
    ...
    if mode == 'k_n':
        ...
        for boolean in bestWorst:
            buffer.append(all_combinations(batch, config, idxEle, kwargs, device = device, largest = boolean)) 
        ...
        for boolean in  bestWorst:
            if config.variable_length:
                ...
                onlyCloudsBatch.append(torch.topk(batch[idxEle,0,:len_element], k = n, largest = boolean)[1].repeat(siz,1))
            else:
                onlyCloudsBatch.append(torch.topk(batch[idxEle][0], k = n, largest = boolean)[1].repeat(siz,1))
        ...
        siz = qnt_clouds-k+1
        batch = batch[idxEle].repeat(siz,1,1)
        elementsToTake = torch.arange(k, qnt_clouds+1, dtype = torch.int64, device = device)
        ...
        for config_topk in bestWorst:
            ...
            for quantity in elementsToTake:
                if config.variable_length:
                    onlyClouds.append(torch.topk(batch[idxEle,0,:len_element], k = quantity, largest = config_topk)[1])
                else:
                    onlyClouds.append(torch.topk(batch[idxEle][0], k = quantity, largest = config_topk)[1])
    ...
    if mode == 'k' or mode == 'n':
        for onlyCloud in onlyCloudsBatch:
            buffer.append(local_error_formula(batch = batch, subsets = subsets, only_clouds = onlyCloud))
    ...
    epsilon = 1e-35
    buffer_normalized = (buffer - minimum) / (maximum - minimum - epsilon) # it's in log(ln)
    point['prn_ln'] = (point['pr_ln'] - minimum) / (maximum -minimum + epsilon)
    point['prn'] = (torch.exp(point['pr_ln']) - torch.exp(minimum)) / (torch.exp(maximum) - torch.exp(minimum) + epsilon)
    ...
    return (pr_error_2graph_ln, pr_error_2graph), buffer_normalized, degradation, minimum, maximum
 



def all_combinations(batch, config, idx_batch= 0, kwargs = None, device = 'cpu', largest = False)-> torch.tensor:
    r'''
    ...
    ks = [torch.arange(start = 2,end = cloud_qnt+1, device = device) for cloud_qnt in cloud_qnts]
    ...
    for cloud_element, ks_element in zip(cloud_qnts, ks):
        ...
        if config.variable_length:
            len_element = kwargs['len_elements'][idx_batch]
            _, index = torch.topk(input = batch[idx_batch,0,:len_element], k = cloud_element, largest = largest) # probable bug

        else:
            _, index = torch.topk(input = batch[idx_batch][0], k = cloud_element, largest = largest) # probable bug
        

        only_clouds = index.repeat(k_sum.shape[0],1) # índices de lo elementos a selecionar
        batch_cloned = batch[idx_batch].clone().unsqueeze(dim=0).repeat(k_sum.shape[0],1,1)
        
        # one element error probability
        pr_error = local_error_formula(batch = batch_cloned, subsets = subsets, only_clouds = only_clouds)
