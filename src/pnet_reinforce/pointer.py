...
class Pointer(nn.Module):
    ...
    def forward(self, encoder_output, encoder_states, kwargs):
        ...
        """
        self.encoder_output = (
            encoder_output  # This will we the ref matrix in the pointer mechanism
        )
        ...
        dec_s = (
            encoder_states  # h_state:[num_layers * num_directions, batch, hidden_size]
        )
        ...
        if self.config.variable_length:
            for i, len_element in enumerate(kwargs["len_elements"]):
                if self.config.mode == "k":
                    self.mask[
                        i, len_element:-1
                    ] = 1  # elements extra(padding) are set to 1 for be non selectionables
                else:
    ...
    ):  # warning cambie el valor aque era sel_nn
        r"""
        For the first element in the batch, in TCP usually uses a city because,
        it's  interpreted as a circular permutation, but in this problem is not 
        the case, for that reason, it's used a synthetic first element.
    ...
    def pointer_mechanism(self, ref_g, q_g, step, kwargs):
        ...
        ref_dot = torch.matmul(
            self.Wref_g, ref_g.permute(1, 2, 0)
        )  # ref_g [steps, batch, hidden_dim] to [batch, hidden_sim, steps]
        self.log.info(
            "tensor self.Wref_g this a learnable parameter: \n %s \n", str(self.Wref_g)
        )
        ...
        self.log.info(
            "Vector u to ponderate ref tensor before to sorfmax \n %s \n", str(vector_u)
        )

        vector_u = F.softmax(vector_u, dim=1)
        self.log.info(
            "vector u to ponderate ref tensor (glimpse funtion) \n %s\n", str(vector_u)
        )

        #   ponderate values
        glimpse_function = ref_g.permute(1, 0, 2) * torch.unsqueeze(
            vector_u, 2
        )  # r_i * p_i
        ...
        ref_dot_pointer = torch.matmul(
            self.Wref, ref_g.permute(1, 2, 0)
        ...
        self.log.info(
            "Values before to matmul with self.vector_v \n %s \n",
    ...
    def pointer_mask(self, step, kwargs):
        ...
        # se bloquea solo la k porque no puede haber una k>n
            ...
            mask_condition = torch.where(
                condition_k_present.reshape(-1, 1), block_q, self.mask
            )
            ...
            stop_condition = self.mask[:, stop_idx:].type(
                torch.bool
            ) + ~condition_k.reshape(
                -1, 1
            )  # bool values
    ...
    def mask_n(self, step, kwargs) -> torch.Tensor:
        r"""
    ...
    def mask_k(self, step, kwargs):
        ...
        if step <= 1:  # Take minimum 2 elements in the batch
            ...
            mask_condition = torch.where(
                stopByItemsSelected.reshape(-1, 1), self.mask, maskPlusBlockStop
            )  # (condition, if true, else)

            conditionStopElement = (
                self.mask[:, stop_idx].type(torch.bool).reshape(-1, 1)
            )
            ...
            mask_condition = torch.where(
                conditionStopElement, mask_clouds, mask_condition
            )
