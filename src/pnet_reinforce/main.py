...
def data_func(generator, config, device, norm = False ,batchSize = None, shape_at_disk = None, replace_element_in_memory  = None) -> dict:
    r''''
    ...
    if  shape_at_disk == 'singleelement' and config.item_in_memory:
        ...
        batch, indices = generator.generate_elements_list(batch_size = batchSize, variable = variable)
    ...
    if config.variable_length:
        indices = map(lambda x: torch.tensor(x, device = device, dtype = torch.int64), indices)
    else:
    ...
    def restriction_data (config, generator, batchSize, device, len_elements, default_restriction = 10):
        if not config.item_in_memory:
            restriction_data = generator.random_number_in_range_of_len_elements(batchSize, len_elements).to(device)
        else:
            if config.variable_length:
                restriction_data = generator.random_number_in_range_of_len_elements(batchSize, len_elements).to(device)
            else:
                restriction_data = torch.tensor([default_restriction], dtype = torch.int64, device = device)
                if config.shape_at_disk == 'batchelements':
                    restriction_data.repeat(20)
        return data

    if config.mode == 'n' and not config.item_in_memory:
        data['restricted_n'] = restriction_data(config,generator,batchSize, len_elements, default_restriction = 10)

    elif config.mode == 'k' and not config.item_in_memory:
        data['restricted_k'] = restriction_data(config, generator, batchSize, len_elements, default_restriction = 2)        

    return data

    

def train_epoch(pointer_net, critic_net, opt_pointer, opt_critic, generator,
                 MSEloss,device, schedulers, config):
    ...
    critic_pred  = critic_net(batch, data) # warning Revisar los elementos para evitar los elementos extra


    #Reward
    if config.mode == 'k':
        reward = Reward(selection, device, batch_steps, config, data)
    else: 
...
def system_evaluation(pointer_net,critic_net, generator,config, device,
                     plot = False, path = None, printOpt = False, shape_at_disk = None):
    
    print('valor de plot {}'.format(plot))
    if shape_at_disk is not None:
        print('valor de config.replace_element_in_memory', config.replace_element_in_memory)
        data = data_func(generator, config, device, config.normal,
                        shape_at_disk = shape_at_disk, replace_element_in_memory = config.replace_element_in_memory)
    else:
        batchSize = 1
    ...
    val_statistic = { 'pr_ln':[], 'prn_ln':[], 'redundancy':[], 'rn':[],'wo':[], 'won':[]}
    
    print('reward dict utilizado en los calulos posterioeres', rewardDict['prError'])

    for index, _ in enumerate(batch):

        # Dict with values of first element of inference and print it in plot to see model performance
        point = {'value_error': rewardDict['prError'][index],
                'prError1': 0,
        ...
        if 'prError' in config.whatToGraph:
            ...
            tuple_pr_error_2graph, pr_normalized_plot, degPr, minimum, maximum=  pr_vals_2_plot(toCompareInPlot, data, point, config, device, index)
            ...
            if plot:
                ...
                if config.log:
                    ...
                    pr_error_2graph = tuple_error_2graph[0] # ln values

                else:
                    points2graph = [(point['pr_error'],'pr')]
                    pr_error_2graph = tuple_error_2graph[1]

                data_and_name_2graph = [(pr_error_2graph, local_names)]
                plotting(data_and_name_2graph , poinst2graph, mode = config.mode, path = pathLocal, logScale = True, labeloptions = labeloptions)

                tuples_data_and_name.append((pr_normalized_plot, local_name))


        if 'redundancy' in config.whatToGraph:
            localNames = ['Redundancia']
            redundancy, redundancy_original, degR, minimum, maximum = reward.redundancyValsPlot(point = point, config = config, kwargs = data, index = index)
            ...
            if plot:
                ...
                plotting([redundancy_original], localNames, points2graph, mode = config.mode, path = pathLocal, labeloptions = labeloptions)
        ...
        if plot:
            ...
            if config.monoObjetive is not None:
                if config.monoObjetive == 'prError':
                    ...
                    raise NotImplementedError('the value in monoObjetive is not valid only is implemented prError and redundancy')
            else:
                points2graph = [(point['wo']), 'WO']


            
            plotting(names, point2graph, mode = config.mode, path = pathComparation, annotatewo = annotatewo)

        if printOpt:
            print('\n\n  Resultados de la experimetacion, número de experimento {}'.format(index))
            print('\n\nSelecton of tuple (k,n):')
            print('\tn quantity: ', reward.sel_n[index])
            print('\tk quantity: ', reward.k_sum[index])
            print('\t --->valor de comparacion (normRed+prError)/2: {}'.format((point['normRed']+point['value_error'])/2))
    ...
    if config.statistic: 
        ...
        for i, (val_k,val_n) in enumerate(zip(reward.k_sum, reward.sel_n)):
            print('Experimento no.{}, valor de la tupla({},{})'.format(i+1,val_k,val_n))
        ...
        for key in val_statistic:
            ...
            if val_statistic[key].shape[0] <= 1:
                print('No se puede realizar operaciones estadisticas con un solo elemento')
            else:
                print('Media, Varianza, Desviación estarndar')
                print('{:.4f}, {:.6f}, {:.4f}'.format(np.mean(val_statistic[key]), np.var(val_statistic[key], ddof = 1), np.std(val_statistic[key], ddof = 1)))
...
def save_model(config, pointer_net, critic_net, opt_pointer, opt_critic):
    torch.save({'model_pointer' : pointer_net.state_dict(),
...
def main():
    ...
    opt_pointer = Adam( pointer_net.parameters(), lr= 1e-03, betas = (0.9, 0.999),
                        eps= 1e-08, weight_decay= config.weight_decay, amsgrad = False)
    opt_critic  = Adam( critic_net.parameters() , lr= 1e-03, betas = (0.9, 0.999), 
                        eps= 1e-08, weight_decay= config.weight_decay, amsgrad = False)
    MSEloss = nn.MSELoss()

    # Learning decay (model optimization) it doesn't work with verbose (Wrappers)
    schedulers = {"pointer": torch.optim.lr_scheduler.StepLR(opt_pointer, step_size = 3, gamma = 0.1, last_epoch = -1), 
                   "critic": torch.optim.lr_scheduler.StepLR(opt_critic, step_size = 3, gamma = 0.1, last_epoch = -1)}
    ...
    if config.train_mode:
        ...
        if config.load_model:
            ...
            pointer_net, critic_net, opt_poiner, opt_critic = load_weight(config, pointer_net, critic_net,
                                                                         opt_pointer, opt_critic, device)
        ...
        for i_epoch in tqdm(range(epochs)):
            ...
            for i in range(stepsByEpochs):
                ...
                train_epoch(pointer_net, critic_net, opt_pointer, opt_critic, 
                            generator, MSEloss, device, schedulers, config)
            ...
            print("\n\n Learning rate in schedulers: \n     Pointer net: {}\n     \ Critic: {}".format(schedulers["pointer"].get_last_lr(), 
            ...
            if evaluation and i_epoch%1 == 0:
                ...
                system_evaluation(pointer_net, critic_net, generator, config, device ,plot = plot,  path = path) 
    ...
    # Inference mode for evaluation
        ...
        system_evaluation(pointer_net, critic_net, generator, config,device,
                         plot = plot, path = path, printOpt = False, shape_at_disk = shape_at_disk)
